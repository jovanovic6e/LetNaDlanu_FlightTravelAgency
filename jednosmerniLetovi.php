...
{
    if(empty($od) || empty($do) || empty($datum))
    {
...
@$datumSQL = mysqli_real_escape_string($konekcija,$_POST["datum"]);
...
<head>
    ...
    <link rel="stylesheet" href="css/jednosmerniLetovi.css?v=<?php echo time(); ?>">
</head>
<body>
        ...
        <form method="post">
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    ...
                    <?php
                    $select_OdJedanSmer = "SELECT DISTINCT pocetnaDestinacija_Grad FROM let WHERE datumDolaska=NULL || datumDolaska=datumPolaska || datumDolaska='1970-01-01'";
                ...
                <div class="col-sm-10">
                    ...
                    <?php
                    $select_DoJedanSmer = "SELECT DISTINCT krajnjaDestinacija_Grad FROM let WHERE datumDolaska=NULL || datumDolaska=datumPolaska || datumDolaska='1970-01-01'";
                ...
                <label for="datum" class="col-sm-2 col-form-label" id="labelInput">Datum:</label>
                <div class="col-sm-10">
                    <input type="date" class="form-control" name="datum" list="datumJedanSmer" min="<?php echo date("Y-m-d"); ?>">
                </div>
            </div>
            <?php
            if(isset($error))
...
<?php
    if(isset($_POST["pretraziLet"]) && !(empty($od) || empty($do) || empty($datum)))
    {
            $prikaziKorisnike = "SELECT * FROM let WHERE pocetnaDestinacija_Grad='$odSQL' && 
            krajnjaDestinacija_Grad='$doSQL' && datumPolaska='$datumSQL' && (datumDolaska=datumPolaska || datumDolaska='1970-01-01' || datumDolaska=null)";
